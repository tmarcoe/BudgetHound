...
public class RegisterController {
	...
	public String saveTransaction(@PathVariable("parent") String parent,
			@Valid @ModelAttribute("register") Register register, BindingResult result, Model model,
			Principal principal) {
		...
		if (register.getCategory().length() > 0) {
			Categories cat = categoriesService.retrieveCategoryByName(household.getHousehold_id(),
	...
	public String listTransaction(@PathVariable("parent") String parent, Model model, Principal principal)
	...
	public String editTransaction(@PathVariable("parent") String parent, @ModelAttribute("entry_id") int entry_id,
	...
	public String upOneTransactions(@PathVariable("parent") String parent, Model model, Principal principal)
			throws ParseException {
		Household household = householdService.retrieve(principal.getName());
	...
	public String updaateTransaction(@PathVariable("parent") String parent,
			@ModelAttribute("register") Register register) {
		registerService.update(register);
		registerService.totalTransaction(register.getHousehold_id(), parent);

		if (register.getCategory().length() > 0) {
			Categories cat = categoriesService.retrieveCategoryByName(register.getHousehold_id(),
					register.getCategory());
	...
	public String deleteTransaction(@PathVariable("parent") String parent, @ModelAttribute("entry_id") int entry_id,
		...
		if (reg.getCategory().length() > 0 && registerService.hasSubCategories(reg.getHousehold_id(), reg.getCategory())) {
			
				registerList = registerService.retrieveList(reg.getHousehold_id(), null, null, parent);
				registerList.setPage(0);
				registerList.setPageSize(15);
				model.addAttribute("error",
						"These transactions have children. Delete the children first before deleting this transaction");
	...
	public String handleRegisterPaging(@PathVariable("parent") String parent, @ModelAttribute("page") String page,
